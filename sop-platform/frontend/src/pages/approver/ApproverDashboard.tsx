...
export default function ApproverDashboard(): JSX.Element {
  ...
  const fetchSops = async (targetPage: number) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setError(res.error || 'Failed to retrieve authorization queue.')
      }
    } catch (err: any) {
      console.error(err)
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '4px' }}>
            Grants final authorization signatures to quality-passed SOP documents.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <p style={{ fontSize: '15px', color: 'var(--text-secondary)', margin: 0 }}>
            Nothing pending. All reviewed SOPs have been processed.
          </p>
        </div>
      ) : (
            ...
            <table style={{
              ...
              <thead>
                <tr style={{
                  ...
                  <th style={{ padding: 'var(--sp-4) var(--sp-6)', fontWeight: 600 }}>QA Approved At</th>
                ...
                {sops.map(sop => (
                  ...
                  >
                    ...
                    <td style={{ padding: 'var(--sp-4) var(--sp-6)', color: 'var(--text-muted)' }}>
                      {formatDate(sop.reviewer_approved_at)}
                    </td>

                    <td style={{ padding: 'var(--sp-4) var(--sp-6)', textAlign: 'right' }}>
                      <button
                        onClick={() => navigate(`/approver/sop/${sop.id}`)}
                        style={{
                          padding: '6px 12px',
